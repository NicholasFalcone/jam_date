...
function Weapon:initByType(t, ammo)
	...
	elseif t == "Molotov" then
		...
		self.Molotov_sfxShot = audioManager:loadSample("sounds/shotgun_shot")
		if self.crosshair then
...
function Weapon:onCrankChangeMolotov(change)
	if not change or change == 0 then return end
	...
	if self.Molotov_lastDir == 0 then
		...
		self.Molotov_currentShakeArc = absc
		self:setState("winding")
		return
	end

	if dir == self.Molotov_lastDir then
		self.Molotov_currentShakeArc = (self.Molotov_currentShakeArc or 0) + absc
		self:setState("winding")
		return
	end
...
function Weapon:triggerMolotovFire()
