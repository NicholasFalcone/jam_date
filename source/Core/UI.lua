...
function UI:init()
    -- Default to HUD so that a UI() created in main.lua and only used for gameplay
    -- will not accidentally render the menu.
...
-- Returns true when it's OK to start gameplay from the menu.
-- This is used by GameManager to gate main.lua's "press A to start" handler
-- without requiring changes to main.lua.
